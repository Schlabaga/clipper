...
def download_clip(clip: str, name ,basepath: str, oauth_token: str, client_id: str) -> None:
    """
    ...
    tempDir = os.path.join(os.path.os.getcwd())+"/clipstemp"
    output_temp_path = tempDir + "/" + out_filename
    ...
    urllib.request.urlretrieve(mp4_url, output_temp_path, reporthook=get_progress)
    
    clipToBeTexted = VideoFileClip(output_temp_path)

    # Generate a text clip  
    txt_clip = TextClip(name, fontsize = 80, color = 'white', font="Amiri-Bold")

    txt_clip = txt_clip.set_position(('left', 'top')).set_duration(clipToBeTexted.duration)

    # Overlay the text clip on the first video clip  
    video = CompositeVideoClip([clipToBeTexted, txt_clip])  

    # Add audio from the original clip
    video = video.set_audio(clipToBeTexted.audio)

    # Write the final video with the text overlay
    output_video_path = basepath + "/" + name + ".mp4"
    video.write_videofile(filename = output_path, codec="libx264", audio_codec="aac")
    
    # Clear the 'clipstemp' directory
    ...
    if "data" in response:
        ...
        for clip in response["data"]:
            ...
            if (
                ...
                and clip["game_id"] == "516575" # VALORANT
                and duration < 60
...
def download_clips(data: dict, path: str, oauth_token: str, client_id: str) -> list:
    ...
    for clip, value in data.items():
        download_clip(value["url"],data[clip]["display_name"], path, oauth_token, client_id)
